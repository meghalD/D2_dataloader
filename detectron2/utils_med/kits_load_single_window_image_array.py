...
def load_multislice_img_16bit_png(data_dir, imname, slice_intv, num_slice):
    ...
    return im,ct_liver_mask_cur



def load_prep_img(data_dir, imname, slice_intv,im_scale, num_slice=3):
    """load volume, windowing, interpolate multiple slices, clip black border, resize according to spacing"""
    ig, ct_liver_mask_cur = load_multislice_img_16bit_png(data_dir, imname, slice_intv, num_slice)
    ig = windowing(ig, [-1024,3072])
    #ig1 = windowing(ig, [-173,275])
    #ig2 = windowing(ig, [-1495,485])
    #ig3 = windowing(ig, [-534,1426])
    # ig1 = windowing(ig, [-600,1400])
    # ig2 = windowing(ig, [-125,225])
    # ig3 = windowing(ig, [-45,105])
    # ig4 = windowing(ig, [-1350,150])
    # ig5 = windowing(ig, [-150,250])
    
    # ig = np.concatenate([ig1,ig2,ig3,ig4,ig5], axis = 2)

    ig_cropped_shape = ig.shape[0:2]


    ig = cv2.resize(ig, None, None, fx=im_scale, fy=im_scale, interpolation=cv2.INTER_LINEAR)
    ct_liver_mask_cur = cv2.resize(ct_liver_mask_cur, None, None, fx=im_scale, fy=im_scale, interpolation=cv2.INTER_LINEAR)

    #print('cropped and resized lesion sum',np.sum(ct_liver_mask_cur) )
    ct_liver_mask_cur1  = copy.deepcopy(ct_liver_mask_cur)

    ct_liver_mask_cur1[ct_liver_mask_cur1>0]  = 1
    #print('cropped and resized lesion sum after>0 = 1',np.sum(ct_liver_mask_cur) )


    #ct_mask_cur = cv2.resize(ct_mask_cur, (current_blackb_crop_shape[1], current_blackb_crop_shape[0]), interpolation=cv2.INTER_CUBIC)
    #ct_mask_zero_cur[c[0]:c[1] + 1, c[2]:c[3] + 1] =  ct_mask_cur
    #ig = ig.astype('uint8')
    #print('shape for ig1 ang ig',ig1.shape,ig.shape)
    return ig,ct_liver_mask_cur1 
